---
title: "Common issues"
description: "Twenty-plus fixes for the questions Voyage merchants run into most often."
---
...
<AccordionGroup>
  <Accordion title="My logo doesn't show in the header">
    ...
    1. **The logo wasn't uploaded.** Theme settings → **Logo** → check both **Logo (default / navy variant)** and **Logo (light variant for dark backgrounds)**. If both are empty, Voyage falls back to the shop name in Playfair Display.
  ...
  <Accordion title="Hero video isn't playing on mobile">
    Mobile autoplay only works under three conditions, all of which Voyage sets by default. If one is broken upstream, autoplay fails silently.

    1. **Muted.** Voyage adds `muted` to every hero video. If you re-encode the MP4 with embedded audio and then strip the attribute, autoplay breaks. Keep the file silent or muted.
    2. **Playsinline.** Voyage adds `playsinline` so iOS doesn't fullscreen the video. Don't remove this attribute.
    3. **Supported format.** MP4 (H.264 / AAC) and WebM are universally supported. AVI, MOV, and HEVC are not.

    File size matters too. Voyage caps hero videos at 4 MB before warning the merchant. Over that, mobile data connections may stall the load and skip autoplay entirely.
  </Accordion>
  ...
  <Accordion title="Cart drawer won't open">
    ...
    Fix: check that **`sections/cart-drawer.liquid`** (or the section referenced in `theme.js`) is present in your theme. If you've deleted it, re-add from the Voyage source. If the request is being blocked by a security app, allowlist the path.
  </Accordion>

  <Accordion title="Customer pages look like generic Shopify pages">
    You're on **New customer accounts**, which renders Shopify-hosted pages instead of your theme's templates. Voyage's `customers/*.liquid` templates only render under **Classic customer accounts**.
  ...
  <Accordion title="Theme styles dropdown only shows Default">
    ...
    Fix: re-upload `config/settings_data.json` from the Voyage source. The file is overwritten safely — your other settings are stored separately.
  </Accordion>
  ...
  <Accordion title="Magnetic CTAs not working">
    ...
    2. **Prefers reduced motion.** macOS and iOS both expose this OS-level preference. Voyage respects it — magnetic CTAs are disabled for visitors who have reduced motion turned on.
  ...
  <Accordion title="Lighthouse perf below 60">
    ...
    Fix: re-export from your image source at JPG q85, max 1800 px on the longest edge. Voyage uses `image_url` with explicit width hints so Shopify can serve smaller sources to smaller viewports — but only if the upload itself isn't oversized.
  </Accordion>

  <Accordion title="Color contrast fails in dark mode">
    The most common cause: a muted-text token that read fine on cream becomes illegible on charcoal. Voyage uses `color_muted` (default `#565E6B`) across both light and dark modes — the same hex.
  ...
  <Accordion title="PDP variant images not swapping">
    Voyage swaps gallery images when a variant is selected — but only if each variant has at least one image assigned in **Products → [product] → Variants**.
  ...
  <Accordion title="Newsletter form not submitting">
    Voyage's newsletter form posts to Shopify's customer-creation endpoint with a `marketing_opt_in` flag. If submissions silently fail:
  ...
  <Accordion title="404 page not found">
    If visiting `/404` returns a generic Shopify 404 instead of Voyage's branded one, you're missing the template.
