...
description: "Shopify Markets, multi-currency, multi-language, RTL, locale files, and money formatting in Voyage."
---

Voyage is built to be sold from anywhere to anywhere. The platform layer — Shopify Markets — handles currencies, languages, taxes, and routing. The theme layer renders the switchers and respects the shopper's choice across every page.
...
Once you've configured more than one currency, Voyage renders a currency switcher in the announcement bar (right side) and in the footer.
...
1. Voyage renders a language switcher in the announcement bar and footer.
...
Voyage ships with `locales/en.default.json` containing every translatable string the theme renders. To support a new language manually:
...
Voyage supports right-to-left languages (Arabic, Hebrew, Persian).

| What Voyage does | What it means |
|---|---|
...
Voyage uses `money` everywhere — the currency code is appended automatically when **Theme settings → Currency → Show currency codes** is checked.
...
<AccordionGroup>
  <Accordion title="My language switcher shows but the page doesn't translate">
    The locale file is missing. Voyage falls back to English if the requested language's locale file doesn't exist. Check `locales/` for the file matching the language code.
  </Accordion>
  ...
  <Accordion title="One section ignores RTL mirroring">
    Likely a section that uses absolute positioning or hand-rolled margins instead of logical properties. Voyage's native sections all use logical properties; if you've added custom sections, audit their CSS.
  </Accordion>
</AccordionGroup>
