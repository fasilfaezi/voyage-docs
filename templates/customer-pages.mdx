...
description: "Voyage's seven editorial customer templates — and the Classic vs New customer accounts trade-off."
---

Voyage ships **seven editorial customer-account templates**. Each is a JSON template wrapping a single `main-customer-*` section, styled in the same vocabulary as the rest of the storefront — script accent on every heading, eyebrow tracked-out caps, serif body, navy ink, paper background.

Before listing them, the most important decision a merchant makes:

## Classic vs New customer accounts

Shopify offers two customer-account modes. The choice is set in admin under **Settings → Customer accounts → Login experience**, and it determines whether Voyage's templates are used at all.

<Tabs>
  <Tab title="Classic">
    The historical Shopify experience. Customers create accounts with email + password, log in, view their order history and addresses. **The storefront renders Voyage's editorial templates** — passport-style order detail, script-prefixed welcome, magnetic CTA buttons.
  ...
  <Tab title="New customer accounts">
    Shopify's modern, passwordless experience. Customers sign in via a one-time code sent to email — no password, no register flow. **Shopify hosts the account UI**, not Voyage. The storefront's templates are bypassed entirely.
...
<Warning>
  Once a merchant enables New customer accounts, Voyage's seven customer templates stop rendering on the storefront. They remain in the codebase — switching back to Classic restores them — but they're inert while New customer accounts is on.
</Warning>
...
The most distinctive customer template in Voyage. The order number renders in the script accent ("VY-12041") as the headline, with "Order detail" as the subtitle — designed to read like a vintage passport stamp.
...
<CardGroup cols={2}>
  <Card title="Sections — customer" href="/sections-reference/main-customer-login">
