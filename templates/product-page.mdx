---
title: "Product page"
description: "Voyage's block-based PDP — fifteen draggable blocks, complementary and related products, gallery and zoom."
---

The product page is the most considered template in Voyage. Its main section is **block-based** — every element in the buy column is a draggable, independent block that the merchant can reorder, hide, or duplicate from the Theme Editor. Three sections compose the page: the main product, complementary products, related products.
...
Iterates `product.options_with_values`. **Color** options render as circular swatches; **Size** options render as text buttons. Anything else renders as a generic size-style row. A subscription-style size-finder tool can be enabled — it cross-references the customer's size at other houses (Loro Piana, Brunello Cucinelli, Zegna, Brioni, Kiton, Drake's, Massimo Dutti, Uniqlo) to suggest a Voyage size.
...
Any Shopify app that ships product-block extensions appears in the **Add block** menu. Voyage renders them at the position chosen.
...
<CardGroup cols={2}>
  <Card title="Main product section" href="/sections-reference/main-product">
