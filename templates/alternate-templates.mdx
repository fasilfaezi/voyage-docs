...
description: "Voyage's three editorial page variants — About, Lookbook, Contact — and how to assign them."
---

Beyond the default `templates/page.json`, Voyage ships three **alternate page templates** — each designed for a specific editorial purpose. They're available to merchants from the **Theme template** dropdown in the page admin, so any page can be reassigned to one of these layouts with a single click.
...
<Steps>
  ...
  <Step title="Open the Theme template dropdown">
    In the page editor's right sidebar, find the section labelled **Theme template**. The dropdown lists every `page.*` template Voyage ships with.
  </Step>
...
The Contact template renders Shopify's built-in `contact` form helper inside Voyage's editorial vocabulary.
