...
description: "The two cursor-aware micro-interactions that give Voyage its tactile feel."
---
...
### Where Voyage applies it

The default magnetic targets in Voyage:
...
On hover-capable devices, Voyage replaces the system cursor with a small dot and a subtle ring that follows your pointer with a slight lag. The lag gives the cursor a physical-feeling weight; the ring grows on hover targets and shrinks on the body.
...
Hover-target elements can change the cursor state by exposing a `data-cursor` attribute. Voyage uses two states:
...
On any device matching `(hover: none)` or `(pointer: coarse)` — phones, most tablets, touch-screen laptops — Voyage skips the cursor entirely. The standard system cursor is used, and the magnetic targets still work via touch (the elements just don't visibly track since there's no hovering pointer).
