...
Voyage's offer popup is a small, considered conversion surface. It's intentionally not a generic "10% off!" overlay — it's framed editorially ("First voyage, fifteen percent.") and stages the conversion in three deliberate steps that mirror how a fashion-led brand would speak to a first-time visitor.
...
| **Any other page** | Triggers after one viewport of scroll on pages without a hero — but Voyage's other templates rarely include the popup snippet, so in practice it's home-only by default. |
...
The headline uses the Voyage script-and-serif typography. The asterisk and "won't appear again on this visit" copy lean into the loss-aversion principle — once the visitor declines, the dismissal is persisted to `sessionStorage` so the claim is honest.
...
The popup includes a `9:16` video in the media frame. The video file is large (~7.9 MB) and would otherwise drag down the home-page LCP. Voyage handles this with lazy attachment:
